...
class SchoolDb2 extends Schema {
  ...
  val entryToComments: _root_.org.squeryl.test.PrimitiveTypeModeForTests.OneToManyRelationImpl[Entry, Comment] = oneToManyRelation(entries, comments).via(
    (e,c) => e.id === c.entryId)
  ...
  val bossToProfessors: _root_.org.squeryl.test.PrimitiveTypeModeForTests.OneToManyRelationImpl[Professor, Professor] =
    oneToManyRelation(professors, professors).
    via((boss,p) => boss.id === p.bossId)

    
  val subjectToParentSubject: _root_.org.squeryl.test.PrimitiveTypeModeForTests.OneToManyRelationImpl[Subject, Subject] =
  ...
  val aToB: _root_.org.squeryl.test.PrimitiveTypeModeForTests.OneToManyRelationImpl[ASTConstructionInterferenceA, ASTConstructionInterferenceB] =
