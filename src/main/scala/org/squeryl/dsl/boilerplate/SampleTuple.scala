...
class SampleTuple
  ...
  override def productArity: Int = outNodes.size
  override def productElement(n: Int):Any = _get(n + 1)

  protected def _get[B](i:Int): B = {
...
object SampleTuple {
  def create(n: List[SelectElement], m: Array[OutMapper[_]]): STuple1[Any] =
    m.length match {
